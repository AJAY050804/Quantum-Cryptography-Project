...
# EDIT SERVER_HOST below to the server machine's IP address
# python client_ui.py
# Open http://localhost:8001  then click CONNECT
...
SERVER_HOST = ""        # <-- PUT SERVER IP HERE e.g. "192.168.1.10"
SERVER_PORT = 5000
BB84_LEN    = 256
ROUNDS      = 20
state       = {"started": False, "sock": None, "aes": None, "done": False, "tb": 0, "tp": 0, "times": []}
...
def run_client():
    ...
    lg(f"Connecting to {host}:{SERVER_PORT}…","info")
    try:
        sk=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        sk.connect((host,SERVER_PORT))
    except Exception as e:
        lg(f"Connection failed: {e}","error")
        state["started"]=False
        socketio.emit('conn_failed')
        return

    state["sock"]=sk
    lg(f"Connected to server at {host}","success")
    ...
    for i in range(ROUNDS):
        ...
        ab=[random.randint(0,1) for _ in range(BB84_LEN)]
        aB=[random.randint(0,1) for _ in range(BB84_LEN)]
        _send(sk,json.dumps({"bits":ab,"bases":aB}).encode())
        bB=json.loads(_recv(sk))["bob_bases"]
        idx=[j for j in range(BB84_LEN) if aB[j]==bB[j]]
        _send(sk,json.dumps({"indices":idx}).encode())
        sifted=[ab[j] for j in idx]
        ...
        srv=json.loads(_recv(sk))
        spub=bytes.fromhex(srv["sign_pub"])
        sxpub=bytes.fromhex(srv["x_pub"])
        sig=bytes.fromhex(srv["signature"])
        ed25519.Ed25519PublicKey.from_public_bytes(spub).verify(sig,sxpub)
        lg("  Ed25519 signature verified","detail")
        ...
        _send(sk,json.dumps({"x_pub":xb.hex()}).encode())
        shared=xp.exchange(x25519.X25519PublicKey.from_public_bytes(sxpub))
    ...
    while True:
        try:
            ...
            socketio.emit('msg',{'who':'Bob','text':pt.decode(),'me':False})
...
def on_chat(data):
    ...
    _send(state["sock"],nonce+ct)
    socketio.emit('msg',{'who':'Alice (you)','text':txt,'me':True})

HTML=r"""<!DOCTYPE html>
<html><head>
<meta charset="UTF-8">
<title>QuantumLink – Client</title>
...
:root{
  --bg:#08080f;--sur:#10101c;--bdr:#202048;
  --g:#7b8cff;--gd:#4455dd;--gdk:#080820;
...
body::after{content:'';position:fixed;inset:0;pointer-events:none;
  background:repeating-linear-gradient(0deg,transparent,transparent 3px,rgba(123,140,255,.007) 3px,rgba(123,140,255,.007) 4px);z-index:999}
...
.role{font-family:var(--fh);font-size:10px;letter-spacing:3px;color:var(--cy);border:1px solid var(--cy);padding:2px 9px}
...
.bub.them{background:#0e0e1c;border:1px solid #202048;border-radius:0 8px 8px 8px;align-self:flex-start}
.bub.me{background:#0a0a1e;border:1px solid #2a2a60;border-radius:8px 0 8px 8px;align-self:flex-end}
.bub .who{font-size:9px;letter-spacing:2px;margin-bottom:3px}
.bub.them .who{color:var(--amb)}
.bub.me .who{color:var(--cy)}
...
<div id="bar">
  ...
  <div class="role">CLIENT · ALICE</div>
...
<div id="body">
  <div id="side">
    <button id="sbtn" onclick="doStart()">▶ CONNECT TO SERVER</button>
    <div>
...
S.on('conn_failed',()=>{
  ...
  document.getElementById('sbtn').textContent='▶ CONNECT TO SERVER';
  document.getElementById('slbl').textContent='FAILED';
});
S.on('round',d=>{
...
function doStart(){
  ...
  document.getElementById('sbtn').textContent='⏳ CONNECTING…';
...
if __name__=='__main__':
    print("Open http://localhost:8001  →  click CONNECT")
